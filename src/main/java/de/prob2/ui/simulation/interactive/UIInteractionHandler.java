...
public final class UIInteractionHandler {
	...
	private DiagramConfiguration.NonUi createUserInteraction(RealTimeSimulator realTimeSimulator, int interactionCounter, List<UIListenerConfiguration> uiListeners) {
		...
		return new ActivationOperationConfiguration(id, Arrays.asList(op), String.valueOf(time), 0, null, ActivationKind.MULTI, fixedVariables, null, TransitionSelection.FIRST, activations, true, null, null, "");
	}
	...
	private List<DiagramConfiguration.NonUi> createActivationConfigurationsFromUserInteraction(List<DiagramConfiguration.NonUi> activationConfigurations, List<DiagramConfiguration.NonUi> userInteractions) {
		...
		if(!hasSetupConstants) {
			...
			if(Transition.SETUP_CONSTANTS_NAME.equals(persistentTrace.getTransitionList().get(0).getOperationName())) {
				...
				activationConfigurationsForResult.add(0, new ActivationOperationConfiguration(Transition.SETUP_CONSTANTS_NAME, Arrays.asList(Transition.SETUP_CONSTANTS_NAME), null, 0, null, ActivationKind.MULTI, fixedVariables, null, TransitionSelection.FIRST, List.of(), true, null, null, ""));
			}
		}
		...
		if(!hasInitialization) {
			...
			activationConfigurationsForResult.add(0, new ActivationOperationConfiguration(Transition.INITIALISE_MACHINE_NAME, Arrays.asList(Transition.INITIALISE_MACHINE_NAME), null, 0, null, ActivationKind.MULTI, fixedVariables, null, TransitionSelection.FIRST, activations, true, null, null, ""));
		}
