		...
		projectNameTextField.setOnKeyPressed(keyEvent -> {
			if (keyEvent.getCode().equals(KeyCode.ENTER)) {
				...
				if(name.replaceAll(" ", "").length() == 0) {
					Alert alert = stageManager.makeAlert(AlertType.WARNING, bundle.getString("project.dialog.error.nameCannotBeEmpty"));
					alert.setHeaderText(bundle.getString("project.emptyName"));
					alert.showAndWait();
					return;
				}
