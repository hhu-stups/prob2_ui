CodeMirror.defineMode("b", function() {
	...
	var operator_x = /\|->|>=|<=(?!>)|%|\*\*|~|\/?<<?:|\/[:=]|<\+|><|\^|<-(?![->])|<<?\||\|>>?|{}|{|}|\[|\]|\|\[\]|<>|(\/\|?\\)|(\\\|?\/)|\'/;
	var operator_w = /(SIGMA|PI|MININT|MAXINT|pred|succ|id|INTER|UNION|card|dom|ran|max|min|union|inter|size|mod|fnc|rel|rev|conc|front|tail|first|last|rec|closure1|closure|iterate|prj[12])/;

	var keyword_w = /MACHINE(\s*\w\w*)?|END\s*\Z|(OPERATIONS|EVENTS|ASSERTIONS|INITIALI[SZ]ATION|SEES|PROMOTES|USES|INCLUDES|IMPORTS|REFINES|EXTENDS|REFINEMENT|CSP_CONTROLLER|SYSTEM|MODEL|IMPLEMENTATION|INVARIANT|CONCRETE_VARIABLES|ABSTRACT_VARIABLES|VARIABLES|PROPERTIES|CONSTANTS|ABSTRACT_CONSTANTS|CONCRETE_CONSTANTS|CONSTRAINTS|SETS|DEFINITIONS)/;
	...
	return {
		...
		token: function(stream, state) {
			...
			if (state.comment === false) {
				...
				if (stream.match(type_w, true) && (stream.eol() || stream.peek().match(/\s/))) {
					return "b-type";
				}
				...
				if (stream.match(logical_w, true) && (stream.eol() || stream.peek().match(/\s/))) {
					return "b-logical";
				}
				...
				if (stream.match(operator_w, true) && (stream.eol() || stream.peek().match(/\s|\(/))) {
					return "b-operator";
				}

				if (stream.match(keyword_w, true) && (stream.eol() || stream.peek().match(/\s/))) {
					return "b-keyword";
				}

				if (stream.match(control_keyword_w, true) && (stream.eol() || stream.peek().match(/\s/))) {
					return "b-controlkeyword";
				}

				if (stream.match(unsupported_w, true) && (stream.eol() || stream.peek().match(/\s/))) {
