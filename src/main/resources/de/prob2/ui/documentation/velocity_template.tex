...
% MACHINE CODE
\section{$converter.latexSafe($machine.getName()) }
	\begin{lstlisting}[caption = $converter.latexSafe($machine.getName()) MCH Code]
...
\subsection{Model Checking}
	...
	$documenter.toUIString($item) & Modelchecking not solved\\
			#else
	$documenter.toUIString($item) & #foreach($result in $item.getItems()) $result.getMessage() #end \\
			#end
		#end
	\tend
	\end{longtable}
...
$converter.latexSafe($sitem.getType().toString()) & $converter.latexSafe($sitem.getCode()) &  $i18n.translate($sitem.getResultItem().getHeaderBundleKey()) \\
			#else
$converter.latexSafe($sitem.getType().toString()) & $converter.latexSafe($sitem.getCode())  & Formula not solved \\
			#end
		#end
	#end
	\tend
