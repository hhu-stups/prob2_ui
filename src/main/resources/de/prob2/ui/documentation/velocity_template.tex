...
The Project Description is: $documentationUtility.latexSafe($project.getDescription())
#end
...
The B Machine $documentationUtility.latexSafe($machine.getName()) is defined by its Code, shown in Listing \ref{lst:code$foreach.index}
\subsection{Code of B Machine $documentationUtility.latexSafe($machine.getName()) }
\begin{lstlisting}[style=MCH, caption = $documentationUtility.latexSafe($machine.getName()) MCH Code, label = lst:code$foreach.index]
...
\subsection{Model Checking}
	...
	\begin{longtable}{*{2}{p{0.5\textwidth}}}
		...
		#foreach($item in $machine.getModelcheckingItems())
			...
			$documentationUtility.toUIString($item,$i18n) & $i18n.translate("verifications.result.notChecked.header")\\
				#else
...
\subsection{LTL Model Checking}
	...
	#set($createDescriptionColumn = $documenter.ltlDescriptionColumnNecessary($machine.getLTLFormulas()))
